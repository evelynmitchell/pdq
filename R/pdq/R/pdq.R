...
{
  if (inherits(self, "ExternalReference")) self = slot(self,"ref") 
  if (inherits(s_sys, "ExternalReference")) s_sys = slot(s_sys,"ref") 
...
{
  if (inherits(self, "ExternalReference")) self = slot(self,"ref") 
  if (inherits(s_term, "ExternalReference")) s_term = slot(s_term,"ref") 
...
{
  if (inherits(self, "ExternalReference")) self = slot(self,"ref") 
  if (inherits(s_batch, "ExternalReference")) s_batch = slot(s_batch,"ref") 
...
{
  if (inherits(self, "ExternalReference")) self = slot(self,"ref") 
  if (inherits(s_trans, "ExternalReference")) s_trans = slot(s_trans,"ref") 
...
# Start of demands_set

`demands_set` = function(s_demands)
{
  s_demands = as.integer(s_demands);
  
  if(length(s_demands) > 1) {
    warning("using only the first element of s_demands");
  };
  
  ;.Call('R_swig_demands_set', s_demands, PACKAGE='pdq');
  
}

attr(`demands_set`, 'returnType') = 'void'
attr(`demands_set`, "inputTypes") = c('integer')
class(`demands_set`) = c("SWIGFunction", class('demands_set'))

# Start of demands_get

`demands_get` = function(.copy = FALSE)
{
  ;.Call('R_swig_demands_get', as.logical(.copy), PACKAGE='pdq');
  
}

attr(`demands_get`, 'returnType') = 'integer'
class(`demands_get`) = c("SWIGFunction", class('demands_get'))
...
{
  if (inherits(s_node, "ExternalReference")) s_node = slot(s_node,"ref") 
...
{
  if (inherits(s_job, "ExternalReference")) s_job = slot(s_job,"ref") 
...
{
  if (inherits(s_Comment, "ExternalReference")) s_Comment = slot(s_Comment,"ref") 
